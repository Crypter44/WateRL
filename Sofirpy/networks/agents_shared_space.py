        ...
        if time > 0:
            if self.time_index % self.control_step_interval == 9:
                ...
                for idx, consumer in enumerate(self.consumer_agents):
                    if len(action) == len(self.all_agents):
                        consumer.set_action(float(action[idx+len(self.pump_agents)]))
                    else:
                        # overwrite calculated demand (demand_volume_flow_m3h) for simpler trainings
